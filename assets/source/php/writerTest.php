...
$file = file_get_contents('../libraries/json/feedbackData.json'); # Открыть файл data.json
...
if ($command === "direction") { # Если это правки направления
  ...
  if (json_encode($direction_index, JSON_UNESCAPED_UNICODE) != 'false') {
    ...
    } else if ($method === "edit_assignee") {
      $taskList[$direction_index]["products"][$product_index]['assignee'] = $assignee;
      echo json_encode($taskList[$direction_index]["products"][$product_index]['assignee'], JSON_UNESCAPED_UNICODE);
    } else {
    ...
    echo "Добавлено новое направление ", $direction_name, " ответственный ", $assignee;
  }
...
file_put_contents("../libraries/json/feedbackData.json", json_encode($taskList, JSON_UNESCAPED_UNICODE)); # Перекодировать в формат и записать в файл.
